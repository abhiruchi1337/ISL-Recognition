...
from orb_processing import descriptor
...
def calc_accuracy(method,label_test,pred):
    ...
    print("precision_score for ",method,sm.precision_score(label_test,pred,average='weighted'))
...
def predict_svm(X, y, X_train, X_test, y_train, y_test):
    ...
    pickle.dump(svc, open('svm_trained_orb.sav','wb'))
    y_pred=svc.predict(X_test)
    calc_accuracy("SVM",y_test,y_pred)
    np.savetxt('submission_orb_svm.csv', np.c_[range(1,len(y_test)+1),y_pred,y_test], delimiter=',', header = 'ImageId,Label,TrueLabel', comments = '', fmt='%d')
...
def predict_lr(X, y, X_train, X_test, y_train, y_test):
    ...
    pickle.dump(clf, open('logreg_trained_orb.sav','wb'))
    y_pred=clf.predict(X_test)
    calc_accuracy("Logistic regression",y_test,y_pred)
    np.savetxt('submission_orb_lr.csv', np.c_[range(1,len(y_test)+1),y_pred,y_test], delimiter=',', header = 'ImageId,Label,TrueLabel', comments = '', fmt='%d')
##    cv = ShuffleSplit(n_splits=10, test_size=0.4, random_state=0)
...
def predict_nb(X, y, X_train, X_test, y_train, y_test):
    ...
    print("======== Naive Bayes ========")
    clf.fit(X_train,y_train)
    pickle.dump(clf, open('naivebayes_trained_orb.sav','wb'))
    y_pred=clf.predict(X_test)
    calc_accuracy("Naive Bayes",y_test,y_pred)
    np.savetxt('submission_orb_nb.csv', np.c_[range(1,len(y_test)+1),y_pred,y_test], delimiter=',', header = 'ImageId,Label,TrueLabel', comments = '', fmt='%d')
...
def predict_knn(X, y, X_train, X_test, y_train, y_test):
    ...
    print("======= KNN =======")
    clf.fit(X_train,y_train)
    pickle.dump(clf, open('knn_trained_orb.sav','wb'))
    y_pred=clf.predict(X_test)
    calc_accuracy("K nearest neighbours",y_test,y_pred)
    np.savetxt('submission_orb_knn.csv', np.c_[range(1,len(y_test)+1),y_pred,y_test], delimiter=',', header = 'ImageId,Label,TrueLabel', comments = '', fmt='%d')
##    cv = ShuffleSplit(n_splits=10, test_size=0.4, random_state=0)
